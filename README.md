...
Две роли учёток: **viewer** видит карту и статусы; **admin** дополнительно
открывает по точке расширенную карточку — история числа клиентов (спарклайн),
история падений и «последний раз была онлайн», атрибуты устройства
(модель/прошивка/аптайм/SSID/диапазон). Коллектор копит историю в SQLite и шлёт
письмо на хелпдеск при падении точки (когда настроен SMTP).
...
npm run add-user -- <логин> <пароль> viewer   # учётка для входа на карту (роль viewer|admin)
```
...
| `HELPDESK_TO` | адрес для писем о падении точек (по умолчанию = `SMTP_TO`) |
| `METRICS_HISTORY_DAYS` | сколько дней хранить историю метрик/событий (дефолт 90) |
| `CLIENT_DROP_THRESHOLD` | порог «массового отключения»: было ≥ N клиентов → стало 0 (дефолт 3) |
...
- Веб наружу отдаёт **только** отфильтрованный статус (`/api/status`): код, имя,
  ...
  Админ-эндпоинт (`/api/admin/point`, только роль `admin`) даёт больше истории и
  атрибутов устройства, но `ip`/`hostid` тоже не отдаёт (резолв `code→hostid` на сервере).
...
public/     фронт без сборки: index.html, map.js, admin.js, styles.css, login.html (+ points.json — генерится)
